	...
	<table id="grid-selection" class="table table-condensed table-hover table-striped">
    		<thead>
       		 <tr>
            		...
            		<th data-column-id="style">舱房类型</th>
...
}).on("loaded.rs.jquery.bootgrid", function()
{
    grid.find(".command-edit").on("click", function(e)
    {
	 ...
	 location.href = "/room/modify/room_id/" + $(this).data("row-id");
...
//	location.href = "/room/remove/room_id" + rows[0];
	$.ajax({
                type: "get",
                url: "/room/remove/room_id/" + rows[0],
                success: function() {
                      //alert('success');
...
function remove1()
{
	...
	//alert(rowIds.join(","));
       $.ajax({
                type: "get",
                url: "/room/remove_selected/room_id/" + rowIds.join(","),
                success: function() {
                      //alert('success');
                  //  editor.insertImage(welEditable, url);
                }

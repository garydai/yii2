...
<div class="panel panel-primary">
  ...
  <button type=button  class="btn btn-success" onclick="location.href =('/boat/add')"><span class="glyphicon glyphicon-plus"></span></button>
        ...
        <table id="grid-selection" class="table table-condensed table-hover table-striped">
			...
			<th data-column-id="p1">载客数/排水量/最高速度/甲板楼层</th>
			<th data-column-id="p2">工作人员/长度/宽度/高度</th>
...
}).on("loaded.rs.jquery.bootgrid", function()
{
    grid.find(".command-edit").on("click", function(e)
    {
         ...
         location.href = "/boat/modify/boat_id/" + $(this).data("row-id");
...
//        alert("You pressed edit on row: " + $(this).data("row-id"));
    }).end().find(".command-delete").on("click", function(e)
    {
        ...
        $.ajax({
                type: "get",
                url: "/boat/remove/boat_id/" + rows[0],
                success: function() {
                      //alert('success');
...
function remove1()
{
       ...
       $.ajax({
                type: "get",
                url: "/boat/remove_selected/boat_id/" + rowIds.join(","),
                success: function() {
                      //alert('success');
                  //  editor.insertImage(welEditable, url);
                }
